...
This is a project that will simulate the behaviour of a closed system with M nodes, and K indistinguishable elements spread throught the M nodes. Where every node can only serve one element at a time, and if there is more than one element in a given node, the rest must wwait in a queueu for the node to serve the first elemnt of the queue. 
